...
- pkgs/
  - python/
    - scope.nix          <Badge text="指定包域" variant="default" size="small" />
    - my-python-pkg/
      - package.nix      <Badge text="callPackage 将使用 scope.nix 指定的 scope 和 args" variant="default" size="small" />
</FileTree>
...
`scope.nix` 也是一个函数，接收全局参数 ([systemContext](../advanced-config)) 并返回配置对象：
...
{ pkgs, ... }:
{
  ...
  # 同级及下级文件中的 callPackage 将从 python311Packages 中查找依赖
  scope = pkgs.python311Packages;
}
```

`pkgs/my-tool/package.nix`:
```nix
