...
*   **模块化管理**
    对于拥有大量 NixOS 模块的项目，Flake FHS 的模块加载机制（尤其是 **子目录模块**）可以帮助你更好地组织代码。
    
    **建议**：确保将独立的模块放入带有 `options.nix` 的子目录中（即 **子目录模块**）。这样，该模块的配置逻辑只有在显式设置 `enable = true` 时才会加载，从而显著减少 Nix 评估时的开销。
