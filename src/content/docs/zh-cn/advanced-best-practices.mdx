...
*   **优先使用子目录包**
    推荐使用**子目录包**（即 `pkgs/<name>/package.nix`），它与 nixpkgs `by-name` 约定兼容，便于贡献上游。该目录下的其他 `.nix` 文件（如 `helper.nix`）不会被自动扫描为独立包，天然支持多文件包并保持对外接口整洁。只有 trivial 的单文件脚本包才用 `pkgs/<name>.nix`。
